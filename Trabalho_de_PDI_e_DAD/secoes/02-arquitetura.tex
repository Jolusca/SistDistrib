...
\subsection{Amazon S3 (Simple Storage Service) e Amazon DynamoDB}
Para garantir a rastreabilidade e a segurança do sistema, todas as tentativas de acesso 
são auditadas utilizando dois serviços complementares da AWS. Assim que a Função Lambda 
recebe a requisição, a imagem do rosto capturado pelo cliente, onde é enviada codificada
no formato Base64, é decodificada e armazenada como evidência visual no Amazon S3 
(Simple Storage Service).

Simultaneamente, os metadados da transação (como data, hora, identificação da porta, 
pontuação de similaridade do Qdrant e o link público para a foto no S3) são persistidos 
no Amazon DynamoDB, um banco de dados NoSQL altamente escalável. Essa abordagem garante 
um histórico de logs imutável e de acesso rápido, permitindo consultas e auditorias 
futuras sem sobrecarregar a camada de processamento vetorial.
